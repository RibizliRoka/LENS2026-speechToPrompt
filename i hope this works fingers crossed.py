...
class RobotClient:
    ...
    def on_press(self,key, injected):
        try:
            ...
            elif key.char == 's' and self.is_recording:
...
def listenForWakeWord(self):
    ...
    with sd.InputStream(samplerate=16000, channels=1, dtype='int16', blocksize=1280) as stream:
        while True:
            ...
            if prediction["hey grippy"] > 0.5 and not self.is_recording:
                self.is_recording = True
                self.startRecording_event.set()
                print("Starting recording...")  

            elif prediction["okay grippy"] > 0.5 and self.is_recording:
                self.is_recording = False
                self.stopRecording_event.set()
                print("Stop recording.....")  
                break

    #copied this from original code 
    ...
    def speechTextToAI(self, speechText):
        load_dotenv()
    ...
    def getAudioPrompt(self):
        ...
        while True:
            ...
            if cmd != "q":
                ...
                ourPrompt = self.speechTextToAI(speechText)
                print(ourPrompt)
