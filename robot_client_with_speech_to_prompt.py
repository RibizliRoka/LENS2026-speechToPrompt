#THIS IS THE SPEECH TO PROMPT VERSION  ]
...
class RobotClient:
    ...
    def __init__(self, config: RobotClientConfig):
        #speech to prompt stuff
        if(config.speechToText == True):
            threading.Thread(target=self.listenForWakeWord, daemon=True).start()
            newTask = self.speechToPromptMain()
            config.setTask(newTask)
    ...
    def listenForWakeWord(self):
        ...
        def speechToPromptMain(self):
            global RECaudio_chunks
            with sd.Stream(samplerate=16000,channels=1,dtype='int16',blocksize=1280,callback=callback):
                self.startRecording_event.wait()
                self.startRecording_event.clear()
                RECaudio_chunks.clear()
                print("recording")
                self.stopRecording_event.wait()
                self.stopRecording_event.clear()
    ...
    def _gripper_open_requested(self) -> bool:
        """Non-blocking check if Enter was pressed on stdin."""
        return select.select([sys.stdin], [], [], 0)[0] != []

    def _open_gripper(self) -> None:
        sys.stdin.readline()
        self.logger.info("Manual gripper open requested — opening gripper")

        # Clear the action queue so the policy stops overwriting our gripper command
        with self.action_queue_lock:
            self.action_queue = Queue()

        # Enable torque and open the gripper
        self.robot.bus.enable_torque("gripper")
        self.robot.bus.write("Goal_Position", "gripper", 100.0)
        self.logger.info("Gripper open — reset the environment then press Enter to resume.")

        # Block here waiting for a second Enter press before resuming inference.
        # During this time the action queue is empty so no policy actions are executed
        # and the gripper stays open.
        input()

        self.logger.info("Resuming inference")
        self.must_go.set()

    def get_current_task(self, default_task: str) -> str:
        """Read task from file if task_file is configured, otherwise use default"""
        task_file = getattr(self.config, 'task_file', None)
        if task_file:
            try:
                from pathlib import Path
                task = Path(task_file).read_text().strip()
                if task:  # Only update if file has content
                    if not hasattr(self, '_last_logged_task') or self._last_logged_task != task:
                        self.logger.info(f"Task updated to: '{task}'")
                        self._last_logged_task = task
                    return task
            except Exception as e:
                self.logger.debug(f"Could not read task file: {e}")
        return default_task
    ...
    def control_loop(self, task: str, verbose: bool = False) -> tuple[Observation, Action]:
        ...
        if task_file:
            try:
                ...
                self.logger.info(f"Initialized task file with: '{task}'")
            except Exception as e:
                self.logger.warning(f"Could not write to task file: {e}")
        ...
        while self.running:
            ...
            """Control loop: (0) Set current task and check if open gripper is requested"""
            current_task = self.get_current_task(default_task)
            #if self._gripper_open_requested():
                #self._open_gripper() #This currently doesn't work, uncomment when it does
